    ...
    if ($existing_count > 0) {
        ...
        $sql = "UPDATE tb_ecobricks
                SET ecobricker_maker=?, volume_ml=?, weight_g=?, sequestration_type=?,
                    ...
                    location_watershed=?, community_id=?, country_id=?, community_name=?
        ...
        $sql = "INSERT INTO tb_ecobricks (
                    ...
                    actual_maker_name, brik_notes, date_published_ts, location_watershed, community_id, country_id, ecobricker_id, community_name
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $gobrik_conn->prepare($sql);
        $stmt->bind_param(
            "issiissssddsssidsdsssssiiis",
            ...
            $brand_name, $owner, $status, $universal_volume_ml, $density, $date_logged_ts,
            $CO2_kg, $last_ownership_change, $actual_maker_name, $brik_notes, $date_published_ts,
            $location_watershed, $community_id, $country_id, $ecobricker_id, $community_name
        );
    }
